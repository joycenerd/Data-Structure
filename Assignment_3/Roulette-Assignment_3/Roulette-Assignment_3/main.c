...
listptr russianRoulette(int dir, listptr ptr, int total) {
...
listptr createList(listptr ptr, char *data, ui len) {
    ...
    // node->mark is to see if the node has been pop
    node->mark = 0;
...
int main()
{
    while (1) {
        ...
        printf("\n");
        // choose the turing direnction
        int dir = directionMenu();
        ptr = russianRoulette(dir, ptr, totalnode);
        // free all node
        listptr rec = ptr;
        listptr temp;
        do {
